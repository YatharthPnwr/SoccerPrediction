...
pub struct Deposit<'info> {
    ...
    #[account(mut)]
    pub vault_team_a: AccountInfo<'info>,

    /// CHECK: This is the vault account for team B
    #[account(mut)]
    pub vault_team_b: AccountInfo<'info>,

    pub system_program: Program<'info, System>,
}

impl<'info> Deposit<'info> {
    pub fn deposit(&mut self, amount: u64, team_name: String) -> Result<()> {
        //Deposit logic here
        ...
        require!(
            self.vault_team_a.key() == self.game_state.vault_team_a
                && self.vault_team_b.key() == self.game_state.vault_team_b,
        ...
        if team_name == self.game_state.team_a_name {
            ...
            self.deposit_sol_to_vault(amount, self.vault_team_a.to_account_info())?;

            //Update the game config PDA
            self.game_state.total_team_a_shares += user_a_shares;
            self.game_state.vault_sol_a += amount;
            ...
            self.deposit_sol_to_vault(amount, self.vault_team_b.to_account_info())?;

            //Update the game config PDA
            self.game_state.total_team_b_shares += user_b_shares;
            self.game_state.vault_sol_b += amount;
            self.game_state.virtual_team_a_pool_tokens = updated_virtual_a_tokens;
            self.game_state.virtual_team_b_pool_tokens = updated_virtual_b_tokens;
